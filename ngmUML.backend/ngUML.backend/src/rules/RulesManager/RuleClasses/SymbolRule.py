...
class SymbolRule(BaseRule):
    ...
    def get_processed_text(self):
        return self.termlist[Classifier, 0].name + "." + self.termlist[Property, 0].name + " " + "CONTAINS" + self.termlist[Value, 0] + " " + self.termlist[Operator, 0]

    def get_validator(self):
        return get_standard_if_statement(
            "len(value) == " + self.termlist[Value, 0], 
            self
        )
