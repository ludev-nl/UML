...
class RelationshipRule(BaseRule):
    ...
    def is_type(termlist):
        return termlist.count(2, 2, 2, 0) and termlist[Operator, 0] in RelationshipRule.operator_keys[0] and termlist[Operator, 1] in RelationshipRule.operator_keys[1]
    ...
    def create_function(self, path):
        ...
        for classifier in path[2:]:
            statement += count * "\t" + "for " + classifier.name.upper() + " in " + parent.name.lower() + "." + classifier.name.lower() + ":\n"
            parent = classifier
            count += 1

        statement += count * "\t" + "sum += " + path[-1].name.upper() + "." + self.termlist[Property, 0].name.lower() + "\n\n"

        statement += "if not (sum " + self.termlist[Operator, 1] + " " + self.termlist[Classifier, 1].name.lower() + "." + self.termlist[Property, 1].name.lower() + "):\n"
