...
class NumericalTwoProperties(BaseRule):
    ...
    def get_processed_text(self):
        return self.termlist[Classifier, 0].name + "." + self.termlist[Property, 0].name + " " + self.termlist[Operator, 0] + " " + self.termlist[Classifier, 0].name + "." + self.termlist[Property, 1].name

    def get_validator(self):
        return get_standard_if_statement_spaces(
            "int(self." + self.termlist[Property, 0].name + ") " + self.termlist[Operator, 0] + " int(self." + self.termlist[Property, 1].name + ")", 
            self
        )
