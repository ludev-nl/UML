...
def add_property_to_add_view(property, application):
    ...
    if property.type == 'bool':
        value2 = ('    if \'' + property.name + '\' in request.POST:\n'
                       + '        ' + property.name + ' = True\n'
                       + '    else:\n'
                       + '        ' + property.name + ' = False\n')
    else:
...
def add_property_to_edit_view(property, application):
    value = '    properties.append({\'type\': \'' + property.type + '\', \'name\': \'' + property.name + '\', \'value\': ' + property.classifier.name.lower() + '.' + property.name + '})\n'
    f = open(application.name + "/views/" + property.classifier.name.lower() + ".py", "r")
    contents = f.readlines()
    ...
    if property.type == 'bool':
        value2 = ('    if \'' + property.name + '\' in request.POST:\n'
                        + '        ' + property.classifier.name.lower() + '.' + property.name + ' = True\n'
                        + '    else:\n'
                        + '        ' + property.classifier.name.lower() + '.' + property.name + ' = False\n')
    else:
        value2 = '    ' + property.classifier.name.lower() + '.' + property.name + ' = request.POST[\'' + property.name + '\']\n'
    f.close()
    contents.insert(index, value)
    contents.insert(index2+1, value2)
...
def delete_from_view(property, application):
    ...
    contents.remove(value)
    if property.type == 'bool':
        value = ('    if \'' + property.name + '\' in request.POST:\n'
                  + '        ' + property.name + ' = True\n'
                  + '    else:\n'
                  + '        ' + property.name + ' = False\n')
    else:
    ...
    if property.type == 'bool':
        ...
        value = '    ' + property.classifier.name.lower() + '.' + property.name + ' = request.POST[\'' + property.name + '\']\n'
    contents.remove(value)

    contructor_index = [i for i, n in enumerate(contents) if n.startswith('    ' + classifier.name.lower() + ' = ' + classifier.name + '(')][0]
