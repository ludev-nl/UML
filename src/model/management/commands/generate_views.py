...
def append_urls(classifier):
    urls = ('    path(\'' + classifier.name.lower() + '/\', views.' + classifier.name.lower() + '.index),\n'
            + '    path(\'' + classifier.name.lower() + '/add\', views.' + classifier.name.lower() + '.add),\n'
            + '    path(\'' + classifier.name.lower() + '/<id>\', views.' + classifier.name.lower() + '.edit),\n'
            + '    path(\'' + classifier.name.lower() + '/<id>/delete\', views.' + classifier.name.lower() + '.delete),\n]\n'
            )
...
def index(classifier, properties):
    index_string = ('def index(request):\n'
                    + '    objects = ' + classifier.name + '.objects.all()\n'
                    + '    for object in objects:\n'
                    + '        object.properties = []\n')
    property_string = '    properties = []\n'
    for prop in properties:
        index_string += ('        object.properties.append({\'name\': \''+ prop.name + '\', \'type\': \'' + prop.type + '\', \'value\': object.' + prop.name + '})\n')
        property_string += '    properties.append({\'name\': \'' + prop.name + '\'})\n'
    index_string += property_string
    index_string += ('    context = {\'properties\': properties, \'objects\': objects, \'object_name\': \'' + classifier.name + '\', \'object_name_lower\': \'' + classifier.name.lower() + '\'}\n'
...
def add(classifier, properties):
    ...
    for prop in properties:
        if prop.type == 'bool':
            add_string += ('    if \'' + prop.name + '\' in request.POST:\n'
                            + '        ' + prop.name + ' = True\n'
                            + '    else:\n'
                            + '        ' + prop.name + ' = False\n')
        else:
...
def delete(classifier):
    ...
    delete_string += classifier.name.lower() + ' = ' + classifier.name + '.objects.get(id=id)\n' \
...
def edit(classifier, properties):
    ...
    for prop in properties:
        edit_string += '    attributes.append({\'type\': \'' + prop.type + '\', \'name\': \'' + prop.name + '\', \'value\': ' + classifier.name.lower() + '.' + prop.name + '})\n'
    ...
    for prop in properties:
        if prop.type == 'bool':
            edit_string += ('    if \'' + prop.name + '\' in request.POST:\n'
                           + '        ' + prop.name + ' = True\n'
                           + '    else:\n'
                           + '        ' + prop.name + ' = False\n')
        else:
...
def generate_crud_views(classifier):
    views_file = open('shared/views/' + classifier.name.lower() + '.py', 'w+')
